...
func setupSVSMSuite(t *testing.T) func() {
	setupCerts(t)
	// Create a new TPM simulator
	sim, err := NewTPMSim(tpm_util.SVSMOnPremiseSNPReportIndex)
	...
	*/
	...
	snp.CheckEnv = func() string { return "SVSM" }
...
func TestSVSMFlow(t *testing.T) {
	t.Cleanup(setupSVSMSuite(t))
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			attribs, err := doSVSMAttestationFlow(t, agentPlugin, serverPlugin)
...
func doSVSMAttestationFlow(t *testing.T, agentPlugin agentnodeattestorv1.NodeAttestorClient, serverPlugin servernodeattestorv1.NodeAttestorClient) (*servernodeattestorv1.AgentAttributes, error) {
	...
	// receive init attestation with SVSM indication
	...
	require.Equal(t, "SVSM", string(agentResponse.GetPayload()), "agent must request an SVSM attestation")
	...
	for {
		...
		if err = agentStream.Send(&agentnodeattestorv1.Challenge{
			...
			return nil, status.Errorf(codes.Internal, "error on send ack to agent: %v", err)
		}

		// receive tpm registration request from agent
		agentResponse, err := agentStream.Recv()
		if err != nil {
			return nil, status.Errorf(codes.Internal, "error on receive tpm registration: %v", err)
		}

		// send tpm registration request to server
		if err = serverStream.Send(&servernodeattestorv1.AttestRequest{
			Request: &servernodeattestorv1.AttestRequest_ChallengeResponse{
				ChallengeResponse: agentResponse.GetChallengeResponse(),
			},
		}); err != nil {
			return nil, status.Errorf(codes.Internal, "error on send tpm registration: %v", err)
		}

		// receive nonce + tpm_challenge from server
		serverResponse, err = serverStream.Recv()
		if err != nil {
			return nil, status.Errorf(codes.Internal, "error on receive nonce and challenge from server: %v", err)
		}

		// send nonce + tpm_challenge to agent
		if err = agentStream.Send(&agentnodeattestorv1.Challenge{
			Challenge: serverResponse.GetChallenge(),
		}); err != nil {
			return nil, status.Errorf(codes.Internal, "error on send nonce and challenge to agent: %v", err)
		}

		// receive attestation_data from agent
		agentResponse, err = agentStream.Recv()
		if err != nil {
			return nil, status.Errorf(codes.Internal, "error on receive attestation data from agent: %v", err)
		}

		// send attestation_data to server
		if err = serverStream.Send(&servernodeattestorv1.AttestRequest{
			Request: &servernodeattestorv1.AttestRequest_ChallengeResponse{
				ChallengeResponse: agentResponse.GetChallengeResponse(),
			},
