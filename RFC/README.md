...
|--------------------------------|----------------------------------------------------------------|---------------------------------------------------|
| `sev_snp:vcek`                 | `sev_snp:vcek:83ed080f561d69119e772579780251b81bd4a433`        | Truncated sha512 of the VCEK.pem (first 20 bytes) |
| `sev_snp:measurement`          | `sev_snp:measurement:d4f53ef416e0daa9a19459b9c97b3371e1454e5c` | Truncated measurment value (first 20 bytes)       |
| `sev_snp:policy:single_socket` | `sev_snp:policy:single_socket:true`                            | false: Guest can be activated on multiple sockets. / true: Guest can be activated only on one socket. |
| `sev_snp:policy:debug`         | `sev_snp:policy:debug:true`                                    | false: Debugging is disallowed. / true: Debugging is allowed. 
| `sev_snp:policy:migrate_ma`    | `sev_snp:policy:migrate_ma:true`                               | false: Association with a migration agent is disallowed. / true: Association with a migration agent is allowed. |
| `sev_snp:policy:smt`           | `sev_snp:policy:smt:true`                                      | false: SMT is disallowed. / true: SMT is allowed. |
| `sev_snp:policy:abi_major`     | `sev_snp:policy:abi_major:1`                                   | The minimum ABI major version required for the guest to run. |
| `sev_snp:policy:abi_minor`     | `sev_snp:policy:abi_minor:50`                                  | The minimum ABI minor version required for the guest to run. |
